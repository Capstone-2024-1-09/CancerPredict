...
def predict_cancer(request):
    if request.method == 'POST':
        ...
        shap_diff = []
        shap_diff_information = ['Age Diff', 'Gender Diff', 'BMI Diff', 'Smoking Diff', 'PhysicalActivity Diff',
                                 'AlcoholIntake Diff', 'CancerHistory Diff']
        ...
        for i in range(0, len(columns)):
            shap_diff.append(
                shap_mean_abs_input_df_final.loc[columns[i], 'Mean_SHAP_Value'] - shap_mean_abs_df_final.loc[
                    columns[0], 'Mean_SHAP_Value'])
